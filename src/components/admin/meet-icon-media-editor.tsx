...
export function MeetIconMediaEditor({ data }: Readonly<{ data: MeetIconMediaEditorData }>) {
  const [groups, setGroups] = useState<readonly MeetIconMediaGroup[]>(() => data.groups.map((group) => ({ ...group, slots: group.slots.map((slot) => ({ ...slot })) })));
  ...
  function updateSlot(key: string, update: (slot: MeetIconMediaSlot) => MeetIconMediaSlot) {
    setGroups((current) => current.map((group) => ({ ...group, slots: group.slots.map((slot) => slot.key === key ? update(slot) : slot) })));
  }

  async function uploadImage(slot: MeetIconMediaSlot, file: File | undefined) {
    if (!file) return;
  ...
  function restoreOriginal(slot: MeetIconMediaSlot) {
    updateSlot(slot.key, (current) => ({ ...current, mediaId: "", media: current.fallbackMedia }));
    setErrors((current) => ({ ...current, [slot.key]: "" }));
  }

  return <form action={saveMeetIconMedia} className={styles.editor}>
    <aside className={styles.guide}>
      <div><strong>{replacementCount}</strong><span>custom images selected</span></div>
      <p>Upload JPG, PNG, WebP or AVIF images, review every preview, then publish all sections together. Empty slots continue using the original approved website image.</p>
    </aside>

    <nav className={styles.sectionNav} aria-label="Meet ICON media sections">
      {groups.map((group) => <a href={`#meet-admin-${group.id}`} key={group.id}><span>{group.number}</span><strong>{group.title}</strong><small>{group.slots.length} {group.slots.length === 1 ? "image" : "images"}</small></a>)}
    </nav>

    {groups.map((group) => <section className={styles.section} id={`meet-admin-${group.id}`} key={group.id}>
      <header className={styles.sectionHeader}>
        <span>{group.number}</span>
        <div><p>Meet ICON media</p><h2>{group.title}</h2><small>{group.description}</small></div>
    ...
    <footer className={styles.actions}>
      <div><strong>Ready to publish?</strong><span>The public page changes only after you save.</span></div>
